...
const adminLogin = async (req, res) => {
    try {
        ...
        // Debug: Print the password and hash
        console.log('Login attempt:', { inputPassword: password, storedHash: admin.password });
            ...
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
                ...
                <div style="background-color: #f8f9fa; padding: 20px; border-radius: 5px; margin-top: 20px;">
                    ...
                    <div style="text-align: center; margin-top: 20px;">
                        <a href="http://localhost:3001/login" style="background-color: #3498db; color: white; padding: 10px 20px; text-decoration: none; border-radius: 5px; display: inline-block;">Login to Dashboard</a>
                    </div>
                </div>
...
// Example Twilio SMS sending function (edit your actual function accordingly)
const sendSMS = async (to, message, twilioClient, fromNumber) => {
    if (!isValidE164(to)) {
        console.error(`❌ Invalid phone number format for SMS: ${to}`);
        return { error: "Invalid phone number format. Must be E.164 format (e.g., +91974584142)." };
    }
    try {
        ...
        console.error(`❌ SMS failed to ${to} { error: "${error.message}", code: ${error.code}, moreInfo: "${error.moreInfo || ''}" }`);
        return { error: error.message, code: error.code, moreInfo: error.moreInfo };
    }
};
